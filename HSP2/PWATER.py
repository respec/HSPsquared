...
from numpy import zeros, ones, sqrt, array, full, nan, argmax, int64, where
from math import log, exp
from numba import njit, types
from numba.typed import Dict
from utilities import initm, hourflag, hoursval
...
def pwater(store, siminfo, uic, ts):
    ''' PERLND WATER module
    ...
    ts['LZETP'] = initm(siminfo, uic, flag,        'MONTHLY_LZETP', u['LZETP'])
    ts['CEPSC'] = initm(siminfo, uic, u['VCSFG'],  'MONTHLY_CEPSC', u['CEPSC'])
    ts['INTFW'] = initm(siminfo, uic, u['VIFWFG'], 'MONTHLY_INTFW', u['INTFW'])
    ts['IRC']   = initm(siminfo, uic, u['VIRCFG'], 'MONTHLY_IRC',   u['IRC'])
    ts['NSUR']  = initm(siminfo, uic, u['VNNFG'],  'MONTHLY_NSUR',  u['NSUR'])
    ts['UZSN']  = initm(siminfo, uic, u['VUZFG'],  'MONTHLY_UZSN',  u['UZSN'])

    # true the first time and at start of every day of simulation
    temp = hourflag(siminfo, 0)
    temp[0] = 1
    ts['DAYFG'] = temp.astype(float)           # numba Dict limitation
