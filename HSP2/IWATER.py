...
from utilities import hourflag, hoursval, initm
...
def iwater(store, siminfo, uic, ts):
    ...
    ts['RETSC'] = initm(siminfo, uic, u['VRSFG'], 'MONTHLY_RETSC', u['RETSC'])
    ts['NSUR']  = initm(siminfo, uic, u['VNNFG'], 'MONTHLY_NSUR',  u['NSUR'])

    # true the first time and at 1am every day of simulation
    temp = hourflag(siminfo, 1)
    temp[0] = 1
    ts['HR1FG'] = temp.astype(float)           # numba Dict limitation
    ...
    ############################################################################
    errors = iwater_(ui, ts)                       # run IWATER simulation code
    ############################################################################
