...
def iwtgas(store, general, ui, ts):
	...
	initm(general, ui, ts, 'WTFVFG', 'AWTFM', 'AWTF')
	initm(general, ui, ts, 'WTFVFG', 'BWTFM', 'BWTF')
	AWTF = ts['AWTF']
	BWTF = ts['BWTF']
	...
	for loop in range(simlen):
		...
		SOHT[loop]   = sotmp * suro   # compute outflow of heat energy in water - units are deg. c-in./ivl
		SODOXM[loop] = sodox * suro   # calculate outflow mass of dox - units are mg-in./l-ivl
		SOCO2M[loop] = soco2 * suro   # calculate outflow mass of co2 - units are mg-in./l-ivl
