...
def graph_from_HDF5(hdfname):
    ...
    links["source"] = links.apply(
        lambda row: f"{row.SVOL}_{row.SVOLNO}", axis="columns"
    )
    links["destination"] = links.apply(
        lambda row: f"{row.TVOL}_{row.TVOLNO}", axis="columns"
    )
    ...
    for x in dg.nodes:
        ...
        if x.startswith("RCHRES"):
            if npred == 0 and nsucc == 0:
                ...
                cm.append("blue")
        elif x.startswith("PERLND"):
            cm.append("green")
        elif x.startswith("IMPLND"):
            cm.append("gray")
        else:
            cm.append("black")
