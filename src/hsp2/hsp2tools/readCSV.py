...
from pandas import HDFStore, read_csv, read_excel
...
def read(csvname, indexcol=None):
    if csvname.lower().endswith(".csv"):
        return read_csv(csvname, index_col=0)
    elif csvname.lower().endswith(".tsv"):
        return read_csv(csvname, index_col=0, delimiter="\t")
    elif csvname.lower().endswith("xlsx"):
        return read_excel(csvname, index_col=0)
    else:
