...
ERRMSGS = (
    "PQUAL: A constituent must be associated with overland flow in order to receive atmospheric deposition inputs",
    "",
)  # ERRMSG0
        ...
        for loop in range(simlen):
            ...
            if QSDFG:
                ...
                if wssd == 0.0:
                    ...
                    washqs = wssd * (
                        sliqsp * slifac + potfw * (1.0 - slifac)
                    )  # lateral inflow has an effect on washoff potency factor
                else:
            ...
            if QSOFG:  # constituent n is simulated by association with overland flow;
                ...
                if (
                    ...
                    if dummy > 1.0:
                        dummy = 1.0
                    sqo = acqop * (delt60 / 24.0) + sqo * (1.0 - dummy) ** (
                        delt60 / 24.0
                    )
                ...
                soqo = 0.0
                if (
                    suro > 0.0 and sqo > 0.0
                ):  # there is overland flow # there is some quality constituent (no. qofp) in storage, washoff can occur
                    dummy = suro * wsfac
                    if dummy < 1.0e-5:
            ...
            if QIFWFG != 0:
                ...
                if ifwo > 0.0:  # there is interflow
                    ioqce = (
                        iliqc * ilifac + ioqc * (1.0 - ilifac) if iliqc >= 0.0 else ioqc
                    )  # lifac not defined, iliqc not defined
                    ioqual = ioqce * ifwo
                else:  # no interflow
                    ioqce = -1.0e30
                    ioqual = 0.0
            ...
            if QAGWFG:  # constituent n is present in groundwater
                ...
                if agwo > 0.0:  # there is baseflow
                    aoqce = (
                        aliqc * alifac + aoqc * (1.0 - alifac) if aliqc >= 0.0 else aoqc
                    )  # kufac bit definedn aliqc bit defubed
                    aoqual = aoqce * agwo
                else:  # no baseflow
                    aoqce = -1.0e30
                    aoqual = 0.0
