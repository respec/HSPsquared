...
ERRMSGS = (
    "IQUAL: A constituent must be associated with overland flow in order to receive atmospheric deposition inputs",
    "",
)  # ERRMSG0
...
def _iqual_(ui, ts):
    ...
    for i in range(nquals):  # simulate constituent
        ...
        for loop in range(simlen):
            ...
            if (
                QSOFG != 0
            ):  #  constituent n is simulated by association with overland flow; the value of qofp refers to the set of overland flow associated parameters to use
                if QSOFG >= 1:  # standard qualof simulation
                    ...
                    ):  # there is some quality constituent (no. qofp) in storage; washoff can occur
