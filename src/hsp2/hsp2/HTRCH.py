...
ERRMSGS = (
    "HTRCH: Water temperature is above 66 C (150 F) -- In most cases, this indicates an instability in advection",
    "",
)  # ERRMSG0
...
def _htrch_(ui, ts):
    ...
    AVDEP = ts["AVDEP"]
    AVDEPE = (
        AVDEP if uunits == 1 else AVDEP * 3.28
    )  # avdepe is the average depth in english units
    ...
    for loop in range(simlen):
        ...
        sovol = SOVOL[loop, :]
        eovol = EOVOL[loop, :]
        tw, roheat, oheat = advect(
            iheat, tw, nexits, svol, vol, srovol, erovol, sovol, eovol
        )  # watertemp treated as a concentration
        svol = vol

        if tw > 66.0:
            if adfg < 2:
                errorsV[0] += (
                    1  # Water temperature is above 66 C (150 F).  In most cases, this indicates an instability in advection.
                )
                rtw = tw
            else:
                tw = tws
        ...
        wind = (
            WIND[loop] * 1609.0
        )  # 5280.0 / 3.28     # get wind movement expressed in m/ivl
        lapse = LAPSE[loop]
        ...
        if avdepe > 0.17:
            ...
            qlongw = (
                4.73e-8
                * ((twkelv**4) - katrad * 1.0e-6 * cldfac * (takelv**6))
                * delt60
                * -1.0
            )
            ...
            if bedflg == 1:  # one-layer bed conduction model
                ...
                cpr = 1000.0  # CPR = density * specific heat of water (and mud); CPR = 1 gm/cm3 * 1 kcal/kg/C * 1000 cm3.kg/m3/g = 1000 kcal/m3/C; this model uses CPR for both water and mud, per Caupp
            ...
            if (
                ...
                spd = spd + kevap * 1.0e-9 * wind * 588750.0 * (
                    0.4436
                    + tw
                    * (
                        28.63195e-3
                        + tw * (0.8e-3 + tw * (0.01124e-3 + tw * 0.00013e-3))
                    )
                )
                spd = 0.5 * spd
        ...
        if uunits == 1:
            ...
            HTEXCH[loop] = htexch * 407960.0 * 12.0 / 43560.0
            ...
            TW[loop] = tw
            AIRTMP[loop] = airtmp
            HTEXCH[loop] = htexch * 1000.0
            ROHEAT[loop] = roheat * 1000.0
            OHEAT[loop] = oheat * 1000.0
            QTOTAL[loop] = qtotal
            QSOLAR[loop] = qsolar
            QLONGW[loop] = qlongw
            QCON[loop] = qcon
...
def vapor(tmp):
    ...
    return 33.8639 * (
        (0.00738 * tmp + 0.8072) ** 8 - 0.000019 * abs(1.8 * tmp + 48.0) + 0.001316
    )
