...
class NUTRX_Class:
    ...
    def __init__(self, siminfo, nexits, vol, ui_rq, ui, ts, OXRX):
        ...
        if (
            self.TAMFG == 1 and self.AMVFG == 1
        ):  # ammonia volatilization parameters table nut-nh3volat
            self.expnvg = ui["EXPNVG"]
            self.expnvl = ui["EXPNVL"]

        if (
            self.TAMFG == 1 and self.PHFLAG == 3
        ):  # monthly ph values table mon-phval, not in RCHRES.SEQ
        ...
        self.RSED1 = zeros(self.simlen)
        self.RSED2 = zeros(self.simlen)
        self.RSED3 = zeros(self.simlen)
        self.RSED4 = zeros(self.simlen)
        self.RSED5 = zeros(self.simlen)
        self.RSED6 = zeros(self.simlen)
        ...
        for i in range(1, 4):
            key = "BNH4" + str(i)
            if key in ui:
                self.bnh4[i] = ui[key] / 1.0e6

            key = "BPO4" + str(i)
            if key in ui:
                self.bpo4[i] = ui[key] / 1.0e6

        self.adnhpm = zeros(4)
        self.adpopm = zeros(4)

        if (self.TAMFG == 1 and self.ADNHFG == 1) or (
            self.PO4FG == 1 and self.ADPOFG == 1
        ):
        ...
        self.dnust2 = zeros(7)
        self.dnust[1] = ui["NO3"]
        self.dnust2[1] = self.dnust[1] * self.vol
        self.dnust[2] = ui["TAM"]
        self.dnust2[2] = self.dnust[2] * self.vol
        self.dnust[3] = ui["NO2"]
        self.dnust2[3] = self.dnust[3] * self.vol
        self.dnust[4] = ui["PO4"]
        self.dnust2[4] = self.dnust[4] * self.vol
        ...
        if (self.TAMFG == 1 and self.ADNHFG == 1) or (
            ...
            for i in range(1, 4):
                self.snh4[i] = (
                    ui["SNH4" + str(i)] / 1.0e6
                )  # suspended nh4 (sand, silt, clay)
                self.spo4[i] = (
                    ui["SPO4" + str(i)] / 1.0e6
                )  # suspended po4 (sand, silt, clay)
        ...
        self.otam = zeros(nexits)
        self.ono2 = zeros(nexits)
        self.opo4 = zeros(nexits)
        self.rosnh4 = zeros(5)
        self.rospo4 = zeros(5)
        self.dspo4 = zeros(5)
        self.dsnh4 = zeros(5)
        self.adpo4 = zeros(5)
        self.adnh4 = zeros(5)
        self.ospo4 = zeros((nexits, 5))
        self.osnh4 = zeros((nexits, 5))
        self.nucf1 = zeros(5)
        self.nucf4 = zeros(8)
        self.nucf5 = zeros(9)
        self.nucf6 = zeros(2)
    ...
    def simulate(
        ...
        self.no3, self.rono3, self.ono3 = advect(
            inno3, self.no3, nexits, self.svol, self.vol, srovol, erovol, sovol, eovol
        )
        ...
        if self.TAMFG == 1:
            ...
            self.tam, self.rotam, self.otam = advect(
                intam,
                self.tam,
                nexits,
                self.svol,
        ...
        if self.NO2FG == 1:
            ...
            self.no2, self.rono2, self.ono2 = advect(
                self.ino2,
                self.no2,
                nexits,
                self.svol,
        ...
        if self.PO4FG == 1:
            ...
            self.po4, self.ropo4, self.opo4 = advect(
                inpo4,
                self.po4,
                nexits,
                self.svol,
        ...
        if self.PO4FG == 1 and self.ADPOFG == 1:
            ...
            for j in range(1, 4):  # get data on sediment-associated phosphate
                ...
                (self.spo4[j], self.dspo4[j], self.rospo4[j], ospo4_) = self.advnut(
                    self.ispo4[j],
                    self.rsed[j],
                    self.rsed[j + 4],
                    depscr[j],
        ...
        if self.TAMFG == 1 and self.ADNHFG == 1:  # advect adsorbed ammonium
            ...
            for j in range(1, 4):
                ...
                (self.snh4[j], self.dsnh4[j], self.rosnh4[j], osnh4_) = self.advnut(
                    self.isnh4[j],
                    self.rsed[j],
                    self.rsed[j + 3],
                    depscr[j],
        ...
        if avdepe > 0.17:
            if self.BENRFG == 1:
                ...
                if self.TAMFG == 1:
                    (self.tam, self.bentam) = benth(
                        OXRX.dox, self.anaer, self.brtam, scrfac, depcor, self.tam
                    )
                    self.bnrtam = self.bentam * self.vol

                if self.PO4FG == 1:
                    self.po4, self.benpo4 = benth(
            ...
            if self.TAMFG == 1:
                if self.AMVFG == 1:  # compute ammonia volatilization
                    ...
                    (self.tam, self.nh3vlt) = self.nh3vol(
                        self.expnvg,
                        self.expnvl,
                        OXRX.korea,
                        wind,
            ...
            if (
                ...
                no3de = 0.0
                (self.no3, no3de) = self.denit(
                    self.kno320, self.tcden, tw, OXRX.dox, self.denoxt, self.no3
                )
            ...
            (self.tam, self.no3, self.po4) = decbal(
                self.TAMFG,
                self.PO4FG,
                self.decnit,
                self.decpo4,
            ...
            if (
                self.PO4FG == 1 and self.SEDFG == 1 and self.ADPOFG == 1
            ):  # compute adsorption/desorption of phosphate
                dumxxx = 0.0

                (self.po4, self.spo4, dumxxx, self.adpo4) = self.addsnu(
                    self.vol,
                    self.rsed,
                    self.adpopm,
                    self.po4,
            ...
            if (
                self.TAMFG == 1 and self.SEDFG == 1 and self.ADNHFG == 1
            ):  # compute adsorption/desorption of ammonium
                # first compute ammonia ionization
                (self.nh3, self.nh4) = self.ammion(tw, self.phval, self.tam)
                (self.nh4, self.snh4, self.tam, self.adnh4) = self.addsnu(
                    self.vol,
                    self.rsed,
                    self.adnhpm,
                    self.nh4,
    ...
    def addsnu(vol, rsed, adpm, dnut, snut, dnutxx, adnut):
        ...
        if vol > 0.0:  # adsorption/desorption can take place
            ...
            for j in range(1, 4):
                if (
                    rsed[j] > 0.0
                ):  # accumulate terms for numerator and denominator in dnut equation
                    num += snut[j] * rsed[j]
                    denom += adpm[j] * rsed[j]
    ...
    def advnut(
        self, isnut, rsed, bsed, depscr, rosed, osed, nexits, rsnuts, rbnuts, bnut
    ):
    ...
    def denit(kno320, tcden, tw, dox, denoxt, no3):
        ...
        if dox <= denoxt:  # calculate amount of no3 denitirified to nitrogen gas
            ...
            if no3 > 0.001:
                ...
                if (
                    no3 < 0.001
                ):  # adjust amount of no3 denitrified so that no3 state variable is not a negative number; set no3 to a value of .001 mg/l
    ...
    def hcintp(self, phval, tw):
        ...
        if (
            tw < 4.44
        ):  # use low temperature range values for henry's constant (4.4 degrees c or 40 degrees f)
            twx = 4.44
        elif (
            tw > 37.78
        ):  # use high temperature range values for henry's constant (37.78 degrees c or 100 degrees f)
            twx = 37.78
        ...
        if (
            ...
            hplus < 1.0
        ):  # use high hydrogen ion concentration range values for henry's constant
            hplus = 1.0
    ...
    def nh3vol(
        self, expnvg, expnvl, korea, wind, delt60, delts, avdepm, twkelv, tw, phval, tam
    ):
    ...
    def nitrif(ktam20, tcnit, tw, no2fg, kno220, tam, no2, no3, dox):
        ...
        if dox >= 2.0:
            ...
            if tam > 0.001:
                ...
                if (
                    tam < 0.001
                ):  # adjust amount of tam oxidized so that tam state variable is not a negative number; set tam to a value of .001 mg/l
                    tamnit = tamnit + tam - 0.001
                    tam = 0.001
            if (
                no2fg == 1
            ):  # calculate amount of no2 oxidized to no3; no2nit is expressed as mg no2-n/l
                no2nit = 0.0
                if no2 > 0.001:
