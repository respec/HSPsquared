...
def cons(io_manager, siminfo, uci, ts):
    ...
    for index in range(ncons):
        ...
        concid = parms[
            "CONCID"
        ]  # string which specifies the concentration units for the conservative constituent.
        ui["conv"] = parms[
            "CONV"
        ]  # conversion factor from QTYID/VOL to the desired concentration units
        qtyid = parms[
            "QTYID"
        ]  # string which specifies the units for inflow or outflow of constituent; e.g. kg
...
def _cons_(ui, ts):
    ...
    for loop in range(simlen):
        ...
        sovol = SOVOL[loop, :]
        eovol = EOVOL[loop, :]
        con, rocon, ocon = advect(
            incon, con, nexits, svol, vol, srovol, erovol, sovol, eovol
        )
