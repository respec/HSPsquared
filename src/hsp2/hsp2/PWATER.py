...
from numpy import argmax, array, full, int64, nan, ones, sqrt, zeros
...
ERRMSGS = (
    "PWATER: Sum of irrtgt in not one",  # ERRMSG0
...
def _pwater_(ui, ts):
    ...
    if uunits == 2:
        ...
        PETMAX = (ts["PETMAX"] * 9.0 / 5.0) + 32.0
        PETMIN = (ts["PETMIN"] * 9.0 / 5.0) + 32.0
    ...
    for step in range(steps):
        ...
        if (
            lperc > 0.0
        ):  #  if necessary, recalculate the fraction of infiltration plus percolation which will be taken by lower zone
