...
def run_Tutorial(hdfname, saveall=False, reloadkeys= False):
        ...
        for opseq in store['/CONTROL/OP_SEQUENCE'].itertuples():
            ...
            for x in sequence[opseq.TARGET]:
                if activity[x.Flag]:
                    ...
                    if 'TUTORIALDATA' in uc:
                        print('Found TUTORIALDATA in ' + opseq.TARGET + ' ' + x.Flag + ', value is ' + str(uc['TUTORIALDATA']))

                    if 'INFILT' in ts:
                        print('Found timeseries for INFILT')
...
def get_timeseries(tsdd, tindex, store):
    ...
    for row in tsdd:
        ...
        if type(row.MFACTOR) == str and row.MFACTOR[0] == '*':
            ...
            print('FOUND timeseries for MFACTOR ' + path)
        else:
